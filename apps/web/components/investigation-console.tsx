...
export function InvestigationConsole({ id }: { id: string }) {
  ...
  return (
    <main id="main" className="shell investigation-console py-12 sm:py-16">
      ...
      {stored && <p className="mb-6 text-xs leading-6 text-muted">{stored.attribution?.status === "ONCHAIN" ? "ONCHAIN ATTRIBUTION / ROBINHOOD TESTNET" : "OFFCHAIN ATTRIBUTION"}. Eligibility remains subject to review.{!stored.privateAccess && " Public metadata only. Unresolved evidence is restricted to authorized human reviewers."}</p>}
      {stored?.attribution?.status === "OFFCHAIN" && stored.attribution.registry && record?.target.chain === "Robinhood Chain Testnet" && <RegisterAttribution target={record.target.address} registry={stored.attribution.registry} originalScout={stored.originatingScout} onConfirmed={() => setAttempt(current => current + 1)} />}
      {stored?.attribution && stored.attribution.status !== "OFFCHAIN" && <section className="mb-6 border border-line p-5 text-xs leading-6"><p className="text-mint">REGISTRY STATUS / {stored.attribution.status}</p><p>CHAIN / Robinhood Chain Testnet / 46630</p>{stored.attribution.registry && <p className="break-all">REGISTRY / {stored.attribution.registry}</p>}{stored.attribution.transactionHash && <p className="break-all">REGISTRATION TRANSACTION / <a className="text-mint underline" href={`https://explorer.testnet.chain.robinhood.com/tx/${stored.attribution.transactionHash}`} target="_blank" rel="noreferrer">{stored.attribution.transactionHash}</a></p>}{stored.attribution.blockNumber && <p>BLOCK / {stored.attribution.blockNumber} / {stored.attribution.confirmationStatus}</p>}{stored.attribution.status === "ONCHAIN" && <p>TARGET ALREADY REGISTERED / Original Scout: {stored.attribution.scout}</p>}{stored.attribution.note && <p className="text-muted">{stored.attribution.note}</p>}{stored.attribution.status === "ONCHAIN" && stored.attribution.scout?.toLowerCase() !== stored.originatingScout.toLowerCase() && <p className="text-muted">Onchain Scout differs from the stored offchain Scout ({stored.originatingScout}). Human review required.</p>}</section>}
