  ...
  return <div className="docs-table-wrap" tabIndex={0} aria-label="Production contract directory">
    <table><caption>Robinhood Chain Mainnet / 4663</caption>
      ...
      <tbody>{Object.entries(deployment.contracts).map(([name, contract]) => <tr key={name}>
        ...
        <td><CopyAddress address={contract.address} /><a href={"https://robinhoodchain.blockscout.com/address/" + contract.address} target="_blank" rel="noreferrer">View on mainnet explorer ↗</a></td>
      </tr>)}</tbody>
    </table>
  </div>;
}
export function DocsArticle({page}:{page:DocPage}){const index=docPages.indexOf(page);return <article><header className="docs-heading"><p className="eyebrow">DOCUMENTATION / {page.group.toUpperCase()}</p><h1>{page.title}</h1><p>{page.description}</p><span className="preview-tag">PUBLIC BETA / HUMAN REVIEW REQUIRED</span></header><nav className="docs-toc" aria-label="On this page"><p className="eyebrow">ON THIS PAGE</p>{page.sections.map(s=><a key={s.title} href={"#"+anchor(s.title)}>{s.title}</a>)}</nav>{page.sections.map(s=><section key={s.title} id={anchor(s.title)}><h2>{s.title}</h2>{s.text?.map(t=><p key={t}>{t}</p>)}{s.pipeline&&<AgentPipeline/>}{s.steps&&<ol className="docs-flow">{s.steps.map(t=><li key={t}>{t}</li>)}</ol>}{s.rows&&<div className="docs-table-wrap" tabIndex={0} aria-label={s.title}><table><caption>{s.title}</caption><tbody>{s.rows.map((row,i)=><tr key={i}>{row.map((cell,j)=>j===0?<th scope="row" key={j}>{cell}</th>:<td key={j}>{cell}</td>)}</tr>)}</tbody></table></div>}{s.links&&<div className="docs-related">{s.links.map(([label,slug])=><Link key={slug} href={"/docs/"+slug}>{label} →</Link>)}</div>}{page.slug==="contracts"&&s.title==="Robinhood Chain Mainnet"&&<Contracts/>}</section>)}{page.slug==="links"&&<ul className="docs-official">{[["WHITEHAT website","https://whitehat.run"],["GitHub repository","https://github.com/TehWhitehat/whitehat"],["Robinhood Chain mainnet explorer","https://robinhoodchain.blockscout.com"],["Robinhood Chain Testnet explorer","https://explorer.testnet.chain.robinhood.com"]].map(([name,url])=><li key={url}><a href={url} target="_blank" rel="noreferrer">{name} ↗</a></li>)}</ul>}<footer className="docs-pagination">{index>0&&<Link href={"/docs"+(docPages[index-1].slug?"/"+docPages[index-1].slug:"")}>← {docPages[index-1].title}</Link>}{index<docPages.length-1&&<Link href={"/docs/"+docPages[index+1].slug}>{docPages[index+1].title} →</Link>}</footer></article>;}
